...
namespace Roborally
{
    ...
    class Board
    {
        ...
        public Board(int w, int h, int s, PictureBox p)
        {
            ...
            Wall w4 = new Wall(this, new Point(0, 0), Direction.Right);
            addFeature(w4);
            Wall w5 = new Wall(this, new Point(0, 1), Direction.Right);
            addFeature(w5);

            // Add bottom wall
            for (int i = 0; i < 6; i++)
            {
                addFeature(new Wall(this, new Point(i, 3), Direction.Down));
            ...
            for (int i = 0; i < 6; i++)
            {
                addFeature(new Wall(this, new Point(i, 0), Direction.Up));
            }

            //Add Right wall
            for (int i = 0; i  < 4; i++)
            {
                addFeature(new Wall(this, new Point(5, i), Direction.Right));
            }

            //Add Left wall
            for (int i = 0; i < 4; i++)
            {
                addFeature(new Wall(this, new Point(0, i), Direction.Left));
            }
            ...
            addFeature(g);
            // Create a Conveyor object at position (1,2) that moves objects 
            Conveyor c = new Conveyor(this, new Point(1, 2), Direction.Right);
            addFeature(c);
            // Create a Conveyor object at position (2,1) that moves objects 
            Conveyor cr = new Conveyor(this, new Point(2, 1), Direction.Right);
            addFeature(cr);
            // Create a Conveyor object at position (4,1) that moves objects 
            Conveyor cu = new Conveyor(this, new Point(4, 1), Direction.Up);
            addFeature(cu);

            Pit deep = new Pit(this, new Point(4, 0), Direction.Right);
            addFeature(deep);

            wallcorner firstWall = new wallcorner(this, new Point(0, 0), Direction.Right);
            addFeature(firstWall);
            wallcorner SecondWall = new wallcorner(this, new Point(5, 0), Direction.Down);
            addFeature(SecondWall);
            wallcorner ThirdWall = new wallcorner(this, new Point(0, 3), Direction.Up);
            addFeature(ThirdWall);
            wallcorner FourthWall = new wallcorner(this, new Point(5, 3), Direction.Left);
            addFeature(FourthWall);
            Winner winner = new Winner(this, new Point(5, 0), Direction.Up);
            addFeature(winner);

            Conveyor Con = new Conveyor(this, new Point(3, 1), Direction.Down, Rotation.clockwise);
            addFeature(Con);
            Conveyor Con1 = new Conveyor(this, new Point(3, 3), Direction.Right, Rotation.widdershins);
            addFeature(Con1);
            Conveyor Con5 = new Conveyor(this, new Point(2, 2), Direction.Right, Rotation.widdershins);
            addFeature(Con5);
            ...
            foreach (Feature ft in myFeature.FullBoard.getBoardSpace(myFeature.Location).getFeatures())
            {
                if (ft.IsMoveable)
                {
                    if (ft is Robot)
                    {
                        ...
                        if (ft.GetType() == typeof(Winner))
                        {
                            MessageBox.Show("Wohoooooo, You, Won!!!");
                        }
                    }
                }
            }
        ...
        public override void performAction()
        {
            foreach (Feature ft in myFeature.FullBoard.getBoardSpace(myFeature.Location).getFeatures())
            {
                if (ft.IsMoveable)
                {
                    if (ft is Robot)
                    {
                        MessageBox.Show("Congratulations, You Won, Chicken Chikcen, Winner Dinner!");
                    }
                }
            }
        }
    ...
    class Conveyor : StationaryFeature
    {
        ...
        public Conveyor(Board f, Point l, Point d, Rotation r) : base(f, l, d)
        {
            ...
            else
            {
                ...
                act = new Turn(Rotation.clockwise);
            }
    ...
    {
        public Winner(Board f, Point l, Point d) : base(f, l, d)
        {
            this.Image = RoboRallyImageSet.winner;
            Action act = new WinBlock();
            act.myFeature = this;
            this.Actions.Add(act);
        }

    }
    class WinnerSquare : StationaryFeature
    {
        public WinnerSquare(Board f, Point l, Point d) : base(f, l, d)
        {
            Winn w = new Winn(true);
            this.Image = RoboRallyImageSet.WinnerBox;
            w.myFeature = this;
            this.Actions.Add(w);
        }
    ...
    class wallcorner : StationaryFeature
    {
        public wallcorner(Board f, Point l, Point d) : base(f, l, d)
        {
            this.Image = RoboRallyImageSet.wallcorner;
        }
    ...
    static class RoboRallyImageSet
    {
        ...
        static public Image ConveyorLeft { get; private set; }
        static public Image ConveyorRight { get; private set; }
        static public Image wallcorner { get; private set; }
        static public Image winner { get; private set; }
        public static Image WinnerBox { get; private set; }


        // Hint B1:  You'll need to add a Pit property for the Pit image
        ...
        public static void loadImages()
        {
            ...
            ConveyorRight.RotateFlip(RotateFlipType.RotateNoneFlipY);
            wallcorner = extract(tileset, 1, 4, transparentColor);
